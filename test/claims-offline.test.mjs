...
test("@claim:offline-demo warmed demo reloads offline and restores the sample", async () => {
  ...
  await context.setOffline(true); await page.reload({ waitUntil: "domcontentloaded" }); await page.locator("#offline").waitFor({ state: "visible" }); await page.locator("#decision-label").getByText("Diverted").waitFor(); await page.locator("#restore-button").click(); await page.locator("#restore-status").getByText(/Restored 1 field/u).waitFor(); assert.ok(await page.evaluate(() => caches.keys()).then((keys) => keys.includes("spillway-shell-v2"))); await context.close();
});
